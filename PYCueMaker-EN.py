...
class MusicEntry:
    def __init__(self, parent, index, remove_callback, move_callback):
        ...
        tk.Label(self.frame, text=f"Music Title: {index + 1}:").grid(row=0, column=0, sticky="w")
        self.title_entry = tk.Entry(self.frame, width=30)
        self.title_entry.grid(row=0, column=1, padx=5, pady=2)

        tk.Label(self.frame, text="Author:").grid(row=0, column=2, sticky="w")
        self.artist_entry = tk.Entry(self.frame, width=20)
        self.artist_entry.grid(row=0, column=3, padx=5, pady=2)

        tk.Label(self.frame, text="File:").grid(row=1, column=0, sticky="w")
    ...
    def browse_file(self):
        filepath = filedialog.askopenfilename(
            title="Browse File",
            filetypes=[("Audio File", "*.mp3 *.wav *.flac *.ogg *.aac"), ("All Files", "*.*")]
        )
        if filepath:
...
class CueGeneratorApp:
    def __init__(self, root):
        ...
        self.album_frame = tk.LabelFrame(self.main_frame, text="Album Imformation", padx=5, pady=5)
        self.album_frame.pack(fill=tk.X, pady=5)

        tk.Label(self.album_frame, text="Album Name:").grid(row=0, column=0, sticky="w")
        ...
        tk.Label(self.album_frame, text="Album Author:").grid(row=1, column=0, sticky="w")
        ...
        self.add_btn = tk.Button(self.main_frame, text="+ Add New Music", command=self.add_entry)
        self.add_btn.pack(pady=5)

        self.generate_btn = tk.Button(self.main_frame, text="Create CUE File", command=self.generate_cue)
    ...
    def remove_entry(self, index):
        if len(self.entries) <= 1:
            self.status_label.config(text="At least need one muisc", fg="red")
            return
    ...
    def update_indexes(self):
        for i, entry in enumerate(self.entries):
            entry.index = i
            entry.title_entry.config(textvariable=tk.StringVar(value=f"Music Title {i + 1}:"))
    ...
    def generate_cue(self):
        ...
        if not album_title:
            messagebox.showerror("Error", "Please Input Album Name")
            return
        ...
        for entry in self.entries:
            ...
            if not data["title"] or not data["file"]:
                self.status_label.config(text="Please Input All Input Field", fg="red")
                return
            tracks.append(data)
        ...
        save_path = filedialog.asksaveasfilename(
            ...
            filetypes=[("CUE File", "*.cue"), ("All Files", "*.*")]
        )
        ...
        try:
            ...
            self.status_label.config(text=f"Success: {save_path}", fg="green")
